...
                return (new Date(cast_detail.birthday)).toDateString().split(' ').slice(1).join(' ');
            });
            const cast_bios = cast_details_array.map(cast_detail => cast_detail.biography);
            const cast_places = cast_details_array.map(cast_detail => cast_detail.place_of_birth);
            return { cast_bdays, cast_bios, cast_places };
        })
        .catch(error => {
            console.error("Error fetching cast details:", error);
            // Handle the error appropriately, e.g., display a user-friendly message
            return { cast_bdays: [], cast_bios: [], cast_places: [] }; // Return empty arrays to prevent further errors
        });
}
